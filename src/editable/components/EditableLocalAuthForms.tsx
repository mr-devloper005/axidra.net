...
const inputClass = 'h-12 rounded-2xl border border-white/30 bg-[#fffaf3] px-4 text-base font-bold text-[#241915] outline-none transition placeholder:text-[#7a655b] focus:border-white focus:bg-white'
const buttonClass = 'inline-flex h-12 items-center justify-center rounded-2xl bg-[#df1f2d] px-6 text-sm font-black uppercase tracking-[0.22em] text-white transition hover:-translate-y-0.5 hover:bg-[#bd1522] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!user || user.password !== password) {
      setStatus('error')
      setMessage('No Axidra account found with these details. Create an account first, then login.')
      return
    }
    saveSession(user)
    setStatus('success')
    setMessage(`Logged in to Axidra as ${user.name}. Redirecting...`)
...
export function EditableLocalSignupForm() {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (password.length < 4) {
      setStatus('error')
      setMessage('Use at least 4 characters for your Axidra password.')
      return
    }
    const users = readUsers()
    const nextUser: LocalUser = {
      name: normalizedName || normalizedEmail.split('@')[0] || 'Axidra User',
    ...
    setStatus('success')
    setMessage('Axidra account created. Redirecting...')
    window.setTimeout(() => router.push('/'), 500)
  }

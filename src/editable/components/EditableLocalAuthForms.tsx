...
const buttonClass = 'inline-flex h-12 items-center justify-center rounded-2xl bg-[var(--editable-page-text,#20252d)] px-6 text-sm font-black uppercase tracking-[0.18em] text-[var(--editable-page-bg,#ffffff)] shadow-sm transition hover:-translate-y-0.5 disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-2xl px-4 py-3 text-sm font-bold ${status === 'success' ? 'bg-[#fdebed] text-[#df1f2d]' : 'bg-red-50 text-red-700'}`}>{message}</p> : null}
